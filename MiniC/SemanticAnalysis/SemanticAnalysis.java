...
public class SemanticAnalysis implements Visitor {
  ...
  public void check(Program progAST) {
    ...
    /* Start of your code: */
    if(null == scopeStack.retrieve("main")) {
      reporter.reportError(errMsg[0], "", progAST.D.pos);
    }
    /* End of your code */
  }
  ...
  public void visit(FormalParamDecl x) {
    ...
    /* Start of your code: */
      if(x.astType.Tequal(StdEnvironment.voidType)) {
          reporter.reportError(x.astType instanceof ArrayType ? errMsg[4] : errMsg[3], "", x.astType.pos);
      }
    /* End of your code */
  }
...
//      scopeStack.closeScope();
  ...
  public void visit(CallStmt x) {
    ...
    //whether e.type can be assigned to t
    public boolean canAssignableTo(Type t, Expr e) {
        if (e instanceof EmptyExpr) {
            return true;
        }
        if (e instanceof ExprSequence) {
            return canAssignableTo(t, ((ExprSequence) e).lAST) && canAssignableTo(t, ((ExprSequence) e).rAST);
        }

        if (t.Tequal(StdEnvironment.intType) && e.type.Tequal(StdEnvironment.floatType)) {
            e = i2f(e);
        }
        return e.type.AssignableTo(t);
    }

    public int getNrOfAstLength(Expr e) {
        int count = -1;
        if (e instanceof EmptyExpr) {
            count = 0;
        } else if (e instanceof ExprSequence) {
            count = getNrOfAstLength(((ExprSequence) e).lAST) + getNrOfAstLength(((ExprSequence) e).rAST);
        } else {
            count = 1;
        }
        return count;
  ...
  public void visit(VarDecl x) {
    ...
    if (!(x.eAST instanceof EmptyExpr)) {
      ...
      if (x.tAST instanceof ArrayType) {
        ...
        if (!(x.eAST instanceof	ExprSequence)) {
            reporter.reportError(errMsg[15], "", x.eAST.pos);
        } else {
            if (!canAssignableTo(typeOfArrayType(x.tAST), x.eAST)) {
                reporter.reportError(errMsg[13], "", x.eAST.pos);
            } else if (getNrOfAstLength(x.eAST) > ((ArrayType) x.tAST).GetRange()) {
                reporter.reportError(errMsg[16], "", x.eAST.pos);
            }
        }
        /* End of your code */
      } else {
        ...
        // Perform i2f coercion if necessary.
          ...
          } else if (x.eAST.type.Tequal(StdEnvironment.intType) && x.tAST.Tequal(StdEnvironment.floatType)) {
              x.eAST = i2f(x.eAST);
          }
          /* End of your code */
      }
    ...
    /* Start of your code: */
      if(x.tAST.Tequal(StdEnvironment.voidType)) {
          reporter.reportError(x.tAST instanceof ArrayType ? errMsg[4] : errMsg[3], "", x.pos);
      }
    /* End of your code */
  }
  ...
  public void visit(VarExpr x) {
    ...
    if(x.Ident.declAST instanceof FunDecl) {
        reporter.reportError(errMsg[11], "", x.Ident.pos);
  ...
  public void visit(BinaryExpr x) {
    ...
    if(HasIntOrFloatArgs(x.oAST)) {
      if(x.lAST.type.Tequal(StdEnvironment.intType) &&
        ...
        /* Start of your code: */
          x.oAST.type = StdEnvironment.floatType;
          x.type = HasBoolReturnType(x.oAST) ? StdEnvironment.boolType : StdEnvironment.floatType;
          x.rAST = i2f(x.rAST);
          /* End of your code */
        return;
      }
  ...
  public void visit(UnaryExpr x) {
    ...
    // slightly more complicated case.
      ...
      if (x.eAST.type.Tequal(StdEnvironment.boolType) && HasBoolArgs(x.oAST)) {
          x.type = StdEnvironment.boolType;
          x.oAST.type = StdEnvironment.intType;
          return;
      }
      ...
      x.type = StdEnvironment.errorType;
      x.oAST.type = StdEnvironment.errorType;
      reporter.reportError(errMsg[10], "", x.eAST.pos);
  ...
  public void visit(CallExpr x) {
    ...
    if(nrOfFormalParams < nrOfActualParams) {
        reporter.reportError(errMsg[23], "", x.paramAST.pos);
    } else if(nrOfFormalParams > nrOfActualParams) {
        reporter.reportError(errMsg[24], "", x.paramAST.pos);
    }
    /* End of your code */
     ...
     */
        ...
        for (int i = 1; i <= GetNrOfFormalParams(F); i++) {
            FormalParamDecl Form = GetFormalParam(F, i);
            ActualParam Act = GetActualParam(x, i);
            Type FormalT = Form.astType;
            Type ActualT = Act.pAST.type;
